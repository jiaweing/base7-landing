...
function extractTextFromBlocks(blocks: BlockObjectResponse[]): string {
  ...
  for (const block of blocks) {
    // Stop reading at the References section
    if (
        ...
        for (const row of rows) {
          if (row.type === "table_row") {
            ...
            text +=
              cells
                .map((cell) =>
                  cell.map((t: any) => toSpeechText(t.plain_text)).join("")
                )
                .join(" | ") + "\n";
          }
        }
...
export function BlogTextToSpeech({ blocks, children }: BlogTextToSpeechProps) {
  ...
  const seekingTimerRef = React.useRef<ReturnType<typeof setTimeout> | null>(
    null
  );
  ...
  const handleWordClick = React.useCallback(
    (absCharIndex: number) => {
      ...
      seekingTimerRef.current = setTimeout(() => setIsSeeking(false), 600);
      seekToChar(absCharIndex);
    },
    [seekToChar]
  );
